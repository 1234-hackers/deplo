...
# app.config['MONGO_URI'] = 'mongodb://'+ipst+':27017/users'
...
@app.route('/peopleass/' , methods = ['POST','GET'])
def peopleass():
    email = session['rset']
    if request.method == "POST":
        users = client.flaka.users
...
@app.route('/login_creator/' , methods = ['POST','GET'])
@login_required
def login_creator():
    if request.method == "POST":# and  hcaptcha.verify():
...
@app.route('/creator_terms/' , methods = ['POST','GET'])
@login_required
def creator_terms():
...
@app.route('/advertising_terms/' , methods = ['POST','GET'])
@login_required
...
@app.route('/request_account/' , methods = ['POST','GET'])
@login_required
def request_account():
     if request.method == "POST":
          ...
          if not ex_t  == "yes":
              ...
              creators.insert_one({'username': username ,  'password':paasc ,'veri':0,'emote': emo ,'plan': plan })
              users.find_one_and_update({"email" : d_user} ,{ '$set' :  {"creator": "yes"}} )
              return 'Account Requested. Check email for further instructions.'
          else:
...
@app.route('/view_prof/' , methods = ['POST','GET'])
@login_required
def view_prof():
...
@app.route('/post_on_tags/' , methods = ['POST','GET'])
...
@app.route('/view_link/' , methods = ['POST','GET'])
@login_required
def view_link():
    link_db = client.flaka.links
    user = client.flaka.users
...
@login_required
def advert():
    advert_db = client.flaka.adverts_request
    if request.method == "POST":
        title = request.form['title']
        link = request.form['link']
        social = request.form['smlink']
        ...
        if countr == '':
           ...
           ctry = countr
           addvert_db.insert_one({"country" : ctry ,'social': social, 'active':1, "title" : title ,'link' : link,'time' : timez })
        return redirect(url_for('feed'))
...
@app.route('/trs/' , methods = ['POST' , 'GET'])
def trs():
